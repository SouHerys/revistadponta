...
export function Reader({ title, pdfUrl, toc }: ReaderProps) {
  ...
  return (
    <div className={`reader-shell ${sidebarOpen ? 'sidebar-open' : ''}`}>
      <div className="reader-toolbar">
        <button className="reader-btn primary toc-open" onClick={() => setSidebarOpen((v) => !v)}><BookOpen size={18} /> Sumário</button>
        <button className="reader-btn icon prev" onClick={() => go(page - 1)}>‹</button>
        <div className="reader-page-box pagebox"><input value={page} onChange={(e) => go(Number(e.target.value || 1))} /> <span>/</span> <span>{pageCount || '?'}</span></div>
      ...
      <div className="reader-main">
        <aside className="reader-sidebar">
          <button className="toc-item" onClick={toggleMark}><strong><Star size={15} /> {marked.includes(page) ? 'Página marcada' : 'Marcar página atual'}</strong><small>Página {page}</small></button>
          {marked.map((p) => <button className="toc-item" key={p} onClick={() => go(p)}><strong>Página marcada</strong><small>Ir para página {p}</small></button>)}
          {toc.length ? toc.map((item) => <button className="toc-item" key={`${item.position}-${item.page}-${item.title}`} onClick={() => go(item.page)}><strong>{item.title}</strong><small>Página {item.page}{item.description ? ` — ${item.description}` : ''}</small></button>) : <div className="empty">Esta edição ainda não tem sumário.</div>}
        </aside>
        <main className="pdf-stage" ref={stageRef} onClick={() => setSidebarOpen(false)} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
          {textMode ? <article className="text-reader">{text.split(/(?<=\.)\s+/).map((p, i) => <p key={i}>{p}</p>)}</article> : <div className="pdf-wrap" ref={wrapRef}><div className="pdf-card"><canvas ref={canvasRef} />{loading && <div className="toast">Carregando página...</div>}</div></div>}
        </main>
      </div>
